...
class energyHub:
    ...
    def __init__(self, parameters, vardata):
        ...
        self.fixdata.psell = 0.05/1000  # k€/kWh
        ...
        self.variables.T        = self.model.addMVar((self.info.num_steps,self.info.num_nodes), lb = -20, ub = 80)
        self.variables.P        = self.model.addMVar((self.info.num_steps,self.info.num_branches), lb = -4000, ub = 4000)  
        self.variables.P_import = self.model.addMVar((self.info.num_steps,self.info.num_nodes), lb=0, ub = 4000)
        self.variables.P_export = self.model.addMVar((self.info.num_steps,self.info.num_nodes), lb=-4000, ub = 0)
        
        # self.variables.x     = self.model.addMVar(self.info.num_nodes, lb = 0, ub = 1, vtype=GRB.BINARY)
        self.variables.V     = self.model.addMVar(self.info.num_nodes, lb=0, ub = 1e8)   # V means size (either m3 or kW)
        self.variables.S     = self.model.addMVar((self.info.num_nodes, 8), lb = 0, ub = 1e4)  # surface area m2 (used for PV and STC, 1 value per each orientation)
         
        self.variables.inv_cost  = self.model.addMVar(self.info.num_nodes, lb=0, ub = 1000)
...
#%% Function that return constraints for each component according to simplified models  
    ...
    def generate_constraints(self):
        ...
        ground_temp = 0*self.vardata['Te'] + 11 #self.undisturbedGround(air_temp)  <------------- need to be changed (use Kusuda formula)
        ...
        for row in self.params['nodes'].iterrows():
            ...
            if node['node_class'] == 'bound-temp':
                ...
                if node['component'] == 'STC':
                    ...
                    if node.isnull()['input1']:
                        self.heatPump_Constraints(node = node,
                                                  bcond2 = locals()[node.input2]) # hot sink
                    elif node.isnull()['input2']:
                        self.heatPump_Constraints(node = node,
                                                  bcond1 = locals()[node.input1]) # cold source
                    else:
                        self.heatPump_Constraints(node = node,
                                                  bcond1 = locals()[node.input1], # cold source
                                                  bcond2 = locals()[node.input2]) # hot sink                                               
...
#%% Surfaces
    ...
    def surface_Constraints(self):
        
        S_max = self.params['surfaces']['max_surf_m2'].values
        nodes = self.params['nodes'].index
        
        self.model.addConstrs((gb.quicksum(self.variables.S[node,orient] for node in nodes) <= S_max[orient] for orient in range(8)), name = 'max_surf_area')      
...
#%% Converters              
    ...
    def solarThermalCollector_Constraints(self, node, solar_irrad, air_temp, wind_vel):
        ...
        U_conv    = (6.9 + 3.9*wind_vel)/1000 # kW/(m2 K) (Kumar & Mullick, 2010)
        
        num_years    = params_stc['constraints_general']['lifetime_years']
        inv_fix      = params_stc['costs']['inv_fix']/1000           # €/m2 --> k€/m2
        inv_per_cap  = params_stc['costs']['inv_per_cap']/1000       # €/m2 --> k€/m2
        ...
        # self.model.addConstrs((self.variables.P[t,branch_down] == gb.quicksum(abs_coeff*self.variables.S[node.name,orient]*solar_irrad.iloc[t,orient] - 
        #                                                                       U_conv[t]*self.variables.S[node.name,orient]*(self.variables.T[t,node.name] - air_temp[t]) 
        #                                                                       for orient in range(8)) for t in range(nt)), name='STC_Eq_1') 
        
        # # 2) Equality constraints: (Linearized) Thermal efficiency of the STC (to determine its power output)
        # self.model.addConstrs((self.variables.P[t,branch_down] == eta_0*gb.quicksum(self.variables.S[node.name,orient]*solar_irrad.iloc[t,orient] -
        #                                                                             a1*self.variables.S[node.name,orient]*(self.variables.T[t,node.name]-air_temp[t])                                                                                    
        #                                                                             for orient in range(8)) for t in range(nt)), name='STC_Eq_2')
        ...
        # gb.quicksum(self.variables.S[node.name,orient]*solar_irrad.iloc[t,orient] for orient in range(8))
        self.model.addConstr(self.variables.inv_cost[node.name] == inv_per_cap_y*gb.quicksum(self.variables.S[node.name,orient] for orient in range(8)), #self.variables.x[node.name]*inv_fix_y +
                             name='STC_Eq_3')
        self.model.update()
        return
    ...
    def heatPump_Constraints(self, node, 
        ...
        heat_source_temp = bcond1  # (cold) heat source temp
        heat_sink_temp   = bcond2  # (hot) heat sink temp
        ...
        num_years    = params_hp['constraints_general']['lifetime_years']
        inv_fix      = params_hp['costs']['inv_fix']/1000           # €/m2 --> k€/m2
        inv_per_cap  = params_hp['costs']['inv_per_cap']/1000       # €/m2 --> k€/m2
        # power_out  = params_wtes['constraints_efficiency']['thermal_power_out']   # kW <-- not yet defined in tech.yaml
        inv_fix_y     = inv_fix/num_years
        inv_per_cap_y = inv_per_cap/num_years
        ...
        if 'HP' in node.component:
            ...
            if heat_source_temp is None:
                # if cold source temperature is not defined, use upstream temperature instead
                self.model.addConstrs((self.variables.P[t,branch_out] ==  self.variables.P[t,branch_el]*(coeff_cop[0] +  coeff_cop[1]*self.variables.T[t,node_in] + coeff_cop[2]*heat_sink_temp[t])  
                                    for t in range(nt)), name='HP-heat_Eq_3') 
            elif heat_sink_temp is None:
                # if hot sink temperature is not defined, use downstream temperature instead
                self.model.addConstrs((self.variables.P[t,branch_out] ==  self.variables.P[t,branch_el]*(coeff_cop[0] +  coeff_cop[1]*heat_source_temp[t] + coeff_cop[2]*self.variables.T[t,node_out])  
                                    for t in range(nt)), name='HP-heat_Eq_3')
            else:
                # if both heat source and heat sink temperatures are defined (linear constraint)  
                self.model.addConstrs((self.variables.P[t,branch_out] ==  self.variables.P[t,branch_el]*(coeff_cop[0] +  coeff_cop[1]*heat_source_temp[t] + coeff_cop[2]*heat_sink_temp[t])  
                                    for t in range(nt)), name='HP-heat_Eq_3')
                
            self.model.addConstrs((self.variables.V[node.name] >= self.variables.P[t,branch_out] for t in range(nt)), name='HP-heat_Eq_4') 
            ...
            self.model.addConstrs((self.variables.P[t,branch_in] >= 0 for t in range(nt)), name='HP-heat_Ineq_6')
            self.model.addConstrs((self.variables.P[t,branch_el] >= 0 for t in range(nt)), name='HP-heat_Ineq_7')
            ...
            if bcond1 is None:
                # if cold source temperature is not defined, use upstream temperature instead (CONSISTENCY TO BE CHECKED!!)
                self.model.addConstrs((self.variables.P[t,branch_in] == self.variables.P[t,branch_el]*(coeff_eer[0] + coeff_eer[1]*self.variables.T[t,node_out] + coeff_eer[2]*heat_sink_temp[t]) 
                                       for t in range(nt)), name='CHILL-heat_Eq_3')
            elif bcond2 is None:
                # if hot sink temperature is not defined, use downstream temperature instead (CONSISTENCY TO BE CHECKED!!)
                self.model.addConstrs((self.variables.P[t,branch_in] ==  self.variables.P[t,branch_el]*(coeff_eer[0] +  coeff_eer[1]*heat_source_temp[t] + coeff_cop[2]*self.variables.T[t,node_out])  
                                    for t in range(nt)), name='CHILL-heat_Eq_3')
            else:
                # if both heat source and heat sink temperatures are defined (linear constraint)  (CONSISTENCY TO BE CHECKED!!)
                self.model.addConstrs((self.variables.P[t,branch_in] ==  self.variables.P[t,branch_el]*(coeff_eer[0] +  coeff_eer[1]*heat_source_temp[t] + coeff_cop[2]*heat_sink_temp[t])  
                                    for t in range(nt)), name='CHILL-heat_Eq_3')
            
            self.model.addConstrs((self.variables.V[node.name] >= self.variables.P[t,branch_in] for t in range(nt)), name='CHILL-heat_Eq_4') 
            # # 4) Inequality constraints:
            # self.model.addConstrs((self.variables.P[t,branch_in] <= Q_ev_max for t in range(nt)), name='CHILL-heat_Ineq_4')        
            self.model.addConstrs((self.variables.P[t,branch_in] >= 0 for t in range(nt)), name='CHILL-heat_Ineq_5')
            self.model.addConstrs((self.variables.P[t,branch_out] >= 0 for t in range(nt)), name='CHILL-heat_Ineq_6')
            self.model.addConstrs((self.variables.P[t,branch_el] >= 0 for t in range(nt)), name='CHILL-heat_Ineq_7')
        
        self.model.addConstr(self.variables.inv_cost[node.name] == inv_per_cap_y*self.variables.V[node.name], name='HP_Eq_5')
        return 
    ...
    def pv_Constraints(self, node, solar_irrad, air_temp, wind_vel):
        ...
        self.model.addConstrs((self.variables.P[t,branch_down] == gb.quicksum(self.variables.S[node.name,orient]*solar_irrad.iloc[t,orient] for orient in range(8))*eta_ref*(
                                1-beta_ref/100*(self.vardata.Te[t]+(9.5/(5.7+3.8*self.vardata.v_wind[t]))*(T_noct-self.vardata.Te[t])*self.vardata.ghi[t]/ghi_noct - T_ref)) 
...
#%% Storage systems  
    ...
    def WTES_Constraints(self, node, bcond1, bcond2):
        ...
        upstream_branches = np.where(self.info.adj[node.name,:] == -1)[0]
        downstream_branches = np.where(self.info.adj[node.name,:] == 1)[0]
        # self.variables.V[node.name]
        # 1) Equality constraints: Thermal balance of the HWT      
        self.model.addConstrs((C*self.variables.V[node.name]*(self.variables.T[t,node.name]-self.variables.T[t-1,node.name]) == 
                               gb.quicksum(self.variables.P[t,upstream_branches]) - gb.quicksum(self.variables.P[t,downstream_branches]) -
                               (a*self.variables.V[node.name] + b)*(bcond2[t]-bcond1[t])   # self.variables.T[t,node.name] replaced by bcond2[t]
                                for t in range(1,nt)), name='WTES_Eq_1') 
        # Adjust constraint (1) for t = 0
        ...
        self.model.addConstr(self.variables.inv_cost[node.name] ==  inv_per_cap_y*self.variables.V[node.name], name='WTES_Eq_2') # self.variables.V[node.name]
        self.model.update()
        return
    ...
    def BTES_Constraints(self, node, ground_temp, air_temp): 
        ...
        sizing_options['SF'] = np.arange(0,1,0.1)
        self.variables.sigma = self.model.addMVar(len(sizing_options.SF), lb = 0, ub = 1, vtype=GRB.BINARY)
        ...
        # Ground properties
        lambdag = params_btes['constraints_efficiency']['lambda_ground']/1000 # W/(m K) --> kW/(m K) 
        ...
        lambdai = params_btes['constraints_efficiency']['lambda_ins']/1000 # W/(m K) --> kW/(m K) 
        inv_fix     = params_btes['costs']['inv_fix']/1000      # €/m3 --> k€/m3
        inv_per_cap = params_btes['costs']['inv_per_cap']/1000  # €/m3 --> k€/m3
        num_years   = params_btes['constraints_general']['lifetime_years']
        inv_fix_y     = inv_fix/num_years
        inv_per_cap_y = inv_per_cap/num_years
        # Pre-sizing: calculation of BTES volume (different sizing options)
        Q_heating_kJ = self.vardata.heat_load.sum()*self.fixdata.time_delta
        eta_seasonal = 0.7
        ...
        if shape == 'Cylindrical':
            ...
            sizing_options['UA_g'] = lambdag*sizing_options.H*HLF  # kW/K                    
        ...
        for i in range(len(sizing_options.SF)): 
            # 1) Thermal balance of the BTES for t = 0
            self.model.addConstr((self.variables.sigma[i] == 1) >> (cv/delta_time*sizing_options.V[i]*(self.variables.T[0,node.name]-self.fixdata.T0[node.name]) - 
                                  gb.quicksum(self.variables.P[0,upstream_branches]) +
                                  gb.quicksum(self.variables.P[0,downstream_branches]) +
                                  sizing_options.UA_a[i]*(self.variables.T[0,node.name]-air_temp[0]) + 
                                  sizing_options.UA_g[i]*(self.variables.T[0,node.name]-ground_temp[0]) == 0), name='BTES_Eq_1')
            
            self.model.addConstr((self.variables.sigma[i] == 1) >> (gb.quicksum(self.variables.P[0,d] for d in downstream_branches) <= sizing_options.UA_x[i]*5), name = 'BTES_Ineq_1')
            self.model.addConstr((self.variables.sigma[i] == 1) >> (gb.quicksum(self.variables.P[0,u] for u in upstream_branches) <= sizing_options.UA_x[i]*5), name = 'BTES_Ineq_2')
                
            for t in range(1,nt):
                # 1) Thermal balance of the BTES for t > 0
                self.model.addConstr((self.variables.sigma[i] == 1) >> (cv/delta_time*sizing_options.V[i]*(self.variables.T[t,node.name]-self.variables.T[t-1,node.name]) -
                                                                        gb.quicksum(self.variables.P[t,upstream_branches]) + 
                                                                        gb.quicksum(self.variables.P[t,downstream_branches]) +
                                                                        sizing_options.UA_a[i]*(self.variables.T[t,node.name]-air_temp[t]) + 
                                                                        sizing_options.UA_g[i]*(self.variables.T[t,node.name]-ground_temp[t]) == 0), name='BTES_Eq_1') 
                ...
                for d in downstream_branches:  
                    ...
                    self.model.addConstr((self.variables.sigma[i] == 1) >> (self.variables.P[t,d] <= sizing_options.UA_x[i]*5),    #(self.variables.T[t,node_down] - self.variables.T[t,node.name])
                                          name = 'BTES_Ineq_1')
                ...
                for u in upstream_branches:  
                    ...
                    self.model.addConstr((self.variables.sigma[i] == 1) >> (self.variables.P[t,u] <= sizing_options.UA_x[i]*5),    #(self.variables.T[t,node_up] - self.variables.T[t,node.name])  <= 0 
                                          name = 'BTES_Ineq_2')
        ...
        self.model.addConstr(self.variables.T[nt-1,node.name]>=self.variables.T[0,node.name], name='BTES_Ineq_1')  #--> THIS CONSTRAIT MAKES THE PROBLEM INFEASIBLE!!
        
        # 3) Equality constraint: choose only one BTES size 
        self.model.addConstr(gb.quicksum(self.variables.sigma[i] for i in range(len(sizing_options.SF))) == 1, name = 'BTES_Eq_2')
        self.model.addConstr(self.variables.V[node.name] == gb.quicksum(self.variables.sigma[i]*sizing_options.V[i] for i in range(len(sizing_options.SF))), name = 'BTES_Eq_3')
        self.model.addConstr(self.variables.inv_cost[node.name] ==  inv_per_cap_y*self.variables.V[node.name], name='BTES_Cost') # self.variables.V[node.name]
        self.model.update()
        self.info.sizing_options_btes = sizing_options
        return
...
#%% General constraints
    ...
    def junction_Constraints(self, node):
        ...
        nodes_down = np.where(self.info.adj[:,downstream_branches] == -1)[0]
        self.model.addConstrs((self.variables.T[t,node.name] >= self.variables.T[t,d] for d in nodes_down for t in range(nt)), name='MinJunctionTemp')
        return
    ...
    def boundaryLoad_Constraints(self, node, bcond1, bcond2 = np.array([])):
        ...
        downstream_branch = np.where(self.info.adj[node.name,:] == 1)[0]
        if len(upstream_branch) > 1:
            print('Boundary nodes must not have more than one upstream branch. Please check hubs.xlsx file')
        if len(downstream_branch) > 1:
            print('Boundary nodes must not have more than one downstream branch. Please check hubs.xlsx file')
        if any(upstream_branch):
            self.model.addConstrs((self.variables.P[t,upstream_branch] == bcond1[t] for t in range(nt)), name='Boundary_Load_Fixed_Heat')
        elif any(downstream_branch):
            self.model.addConstrs((self.variables.P[t,downstream_branch] == bcond1[t] for t in range(nt)), name='Boundary_Load_Fixed_Cool')       
        if bcond2.any() == True:
...
#%% Objective function            
    ...
    def objective_function(self, co2_price = 150):
        ...
        #                         self.fixdata.time_delta/3600*self.fixdata.pbuy*gb.quicksum(self.variables.P_import[t,n] for n in range(nn) for t in range(nt)) + 
        ...
        co2_price = co2_price/1000  # conversion €/t --> €/kg
        self.model.setObjective(gb.quicksum(self.variables.inv_cost[n] for n in range(nn)) +
                                self.fixdata.time_delta/3600*self.fixdata.pbuy*gb.quicksum(self.variables.P_import[t,n] for n in range(nn) for t in range(nt)) + 
                                self.fixdata.time_delta/3600*self.fixdata.psell*gb.quicksum(self.variables.P_export[t,n] for n in range(nn) for t in range(nt)) +
                                co2_price*self.fixdata.time_delta/3600*gb.quicksum(self.vardata.CO2_intensity[t]/1000*self.variables.P_import[t,n] for n in range(nn) for t in range(nt)), 
                                gb.GRB.MINIMIZE)
        
        # last term divided by 1000 to convert CO2_intensity of electricity from g/kWh in kg/kWh
        self.model.update()
        return  
    
    
    def run(self, timeLimit = 60, MIPGap = 0.001):
        ...
        try:
            ...
            # self.model.params.FeasibilityTol  = 1e-4
...
#        # Load updated variables in tseries object (for controller check)
        ...
        self.output['surf']         = self.variables.S[:].x         # ['N','NE','E','SE','S','SW','W','NW'] from SolarClasses
